...
public class AmethystVeinFeature extends Feature<AmethystVeinFeatureConfig> {
    ...
    private Vec3d blockPosToVec3d(BlockPos blockPos) {
        return new Vec3d(
            blockPos.getX() + 0.5,
            blockPos.getY() + 0.5,
            blockPos.getZ() + 0.5
    ...
    private BlockPos vec3dToBlockPos(Vec3d vec) {
        return new BlockPos(
            (int) vec.getX(),
            (int) vec.getY(),
            (int) vec.getZ()
    ...
    private void setOutline(StructureWorldAccess world, ArrayList<Long> outline, Random random) {
        for (long l : outline) {
            ...
            this.setBlockState(world, pos, OUTLINE_STATE_PROVIDER.get(random, pos));
        }
    }
    ...
    private void setAmethyst(StructureWorldAccess world, BlockPos pos, Random random, boolean undergroundOnly) {
        ...
        BlockState state = undergroundOnly ? Blocks.AMETHYST_BLOCK.getDefaultState() : SUSPENDED_STATE_PROVIDER.get(random, pos);
